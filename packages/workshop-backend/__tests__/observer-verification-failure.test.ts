...
describe("a binding that fails verification", () => {
  ...
  it("serializes returning observer opens and preserves registrations after a failed open", async () => {
    ...
    await runInDurableObject(stub, async (instance: OverseerDurableObject) => {
      ...
      let removed: number[] = [];
      // The fork serializes the whole admission per observer. Call B must stay
      // queued while A's verifier is parked, then retry after A is denied.
      ...
      impl.getGatekeeperFacet = (id: number) => ({
        addObserver: async () => {
          if (id === 3 && ++gk3Adds <= 2) throw new Error("no access");
        },
        removeObserver: async () => { removed.push(id); },
      });
      let configureCb = { configure: async (needs: {gatekeeperId: number}[]) =>
      ...
      let callBConfigured = false;
      let callB = impl.ensureObserver("alice", fakeClientUser, "build", {
        configure: async (needs: {gatekeeperId: number}[]) => {
          callBConfigured = true;
          return configureCb.configure(needs);
        },
      });
      callB.catch(() => {});
      // Yield one real event-loop turn: no clock arithmetic under workerd.
      await new Promise(resolve => setTimeout(resolve, 0));
      expect(callBConfigured).toBe(false);
      expect(gk3Adds).toBe(0);
      releaseVerifier();
      await expect(callA).rejects.toThrow(/could not confirm/);
      await callB;
      expect(callBConfigured).toBe(true);
      expect(gk3Adds).toBe(3);
      // Returning observers share a persisted registration id; A's failure must
      // preserve it so B can finish with all three connections admitted.
      expect(removed).toEqual([]);
