...
describe("authorizeObservation's containsRestrictedData and ownerInvitesOnly flags", () => {
  ...
  it("revokes access for people the owner did not add directly when ownerInvitesOnly is set",
    ...
    await runInDurableObject(stub, async (instance: OverseerDurableObject) => {
      ...
      // cleanup off the network. Fork: the latch revokes through the gated revocation restart
      // (see runRevocationCleanup), like removeCollaborator, so that is the seam stubbed here.
      let restarts: string[] = [];
      impl.scheduleRevocationRestart = (_block?: boolean, reason?: string) => {
