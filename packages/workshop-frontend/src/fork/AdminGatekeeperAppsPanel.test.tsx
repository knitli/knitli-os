...
vi.mock('../SandboxedGatekeeperApp', () => ({ default: (props: { title?: string }) => { received(props); return <iframe title={props.title} /> } }))

describe('AdminGatekeeperAppsPanel', () => {
  let root: Root | undefined
  let container: HTMLDivElement | undefined

  afterEach(() => { act(() => root?.unmount()); container?.remove(); vi.restoreAllMocks(); dispose.mockClear(); received.mockClear() })
  ...
  it('B-HOST-005 renders the accessible lifecycle and disposes a closed frame once', async () => {
    ...
    await act(async () => root!.render(<StrictMode><AdminGatekeeperAppsPanel admin={admin} /></StrictMode>))
    ...
    expect(received).toHaveBeenLastCalledWith(expect.objectContaining({ title: 'OpenAPI segments administration' }))
    expect(container!.querySelector('[role="region"]')?.getAttribute('aria-label')).toBe('OpenAPI segments administration')
