...
class OverseerImpl implements AgentHooks {
  ...
  // Consumed references may not be in chat history yet when approval arrives.
  #consumedApprovalWaiters = new Map<number, { boundary: number; actionIds: number[] }>();
  approvalWaiters(chatId: number) {
    return currentApprovalWaiters(this.storage.chats.list({prefix: `${keyString(chatId)}.`, reverse: true}),
      id => this.storage.actions.get(id));
  }
  ...
  async authorizeObservation(gatekeeperId: number, description: ObservationDescription,
    ...
    // Only the fork's owner-only tier blocks when shared. Restricted data no longer does
    // (upstream #382): observer verification at admission governs who sees it.
    let sharing = prohibitWorkspaceSharing ||
    ...
    // Revocation-pause only, not the full observer readiness: an unrelated legacy connection
    // must not turn the observation path into an error (see #inRoleVerificationScope), and
    // collaborator coverage is enforced at admission, not per read (upstream #382).
    ...
    let baseline = sharing && description.ownerInvitesOnly && !this.storage.ownerInvitesOnly.get()
    ...
    // the check and the sticky state transition. Only the fork's owner-only tier blocks here: an
    // ownerInvitesOnly observation is meant to latch in a shared workspace, narrowing it.
    if (prohibitWorkspaceSharing && sharing?.hasAnyShares()) {
      throw new Error(
    ...
    if (sharing && baseline) {
      ...
      if (affected.length > 0) {
        ...
        // above. Fork: routed through runRevocationCleanup like every other revocation path, so
        // the restart holds the input gate (see scheduleRevocationRestart); the cleanup is
        // best-effort and not awaited, and whatever the restart cuts off self-heals.
        runRevocationCleanup({
          tearDownObservers: () => this.tearDownLostObservers(affected),
          refreshListings: () => this.refreshAffectedCollaboratorListings(affected),
          scheduleRestart: () => this.scheduleRevocationRestart(true,
  ...
  // - The abort is delayed briefly so the triggering RPC's response can reach the caller
  //   (typically the owner, who is also connected and will be disconnected) before the
  //   connection drops. Without it their own removeCollaborator()/revokeShareLink() call
  //   can reject with a connection error even though it succeeded. Restored from upstream:
  //   the gate rewrite in a811be9 dropped it, which broke workspace deletion round-trips.
  scheduleRevocationRestart(blockNewInputs = true, reason?: string): void {
    if (this.#revocationRestartScheduled) return;
    this.#revocationRestartScheduled = true;
    let restart = async () => {
      await this.ctx.storage.sync();
      await scheduler.wait(100);
      this.ctx.abort(reason ?? "Gadget restarted to revoke access for a removed collaborator.");
    };
  ...
  async #runAgentTurnWithContext(chatId: number, aiModel: UserAiModelRecord,
    ...
    try {
      ...
      // Approval may win before captured references reach durable history. Recheck
      // the normally suspended turn after its step commits, without another await.
      ...
      // An approval summary restarts the agent so it reacts promptly -- unless calls were
      // recorded mid-turn, which drain first (and see the summary in history). Otherwise the
      // turn ends through the shared hand-off, which delivers any waiting response.
      if (summary && approvalAuthor && meta && activeRecord &&
          !this.hasPendingAgentCalls(chatId)) {
  ...
  consumeCapturedActions(chatId: number)
    ...
    if (result?.awaitDecision) {
      const waiters = this.approvalWaiters(chatId);
      if (waiters) this.#consumedApprovalWaiters.set(chatId, {boundary: waiters.boundary, actionIds: result.actions});
    }
    return result;
  }
  ...
  // The fork's owner-only tier: a latched owner-only observation or an owner-only
  // connection. Restricted data is deliberately NOT part of this predicate (upstream #382):
  // observer verification at admission governs who sees it, and such workspaces stay shareable.
  isWorkspaceSharingProhibited(): boolean {
    return this.storage.prohibitWorkspaceSharing.get() ||
  ...
  // Readiness gate for action/hook paths (observations take only the revocation pause inline):
  // refuse while a revocation is mid-flight (a revoked collaborator's session is still live until
  // the restart lands). Collaborator coverage is NOT checked here (upstream #382): a scope
  // widening restarts every live session instead, and each collaborator re-verifies at their own
  ...
  async getSharingManager(): Promise<SharingManager> {
    if (!this.#sharingManager) {
      this.#sharingManager = new SharingManager(
        ...
        () => this.storage.ownerInvitesOnly.get(),
...
export class OverseerDurableObject extends DurableObject<Cloudflare.Env> {
  ...
  async open(userId: string, profileId: string,
    ...
    if (!isOwner) {
      if (this.impl.isWorkspaceSharingProhibited()) {
        // An owner-only latch or connection can only exist while the gadget has no shares (see
        // `authorizeObservation` and the connection guard), and no new shares can be created
        // while either is set, so any non-owner reaching here is necessarily unauthorized.
  ...
  async receiveExternalMessage(
    ...
    if (ownerId !== callerId) {
      ...
      collaboratorSharing = await this.impl.getSharingManager();
      if (role !== "build") {
        return {
          accepted: false,
...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async #maybeResumeAfterActionDecision(chatId: number): Promise<void> {
    ...
    const waiters = this.impl.approvalWaiters(chatId);
    const summary = approvedActionSummary(waiters?.actions);
    if (!waiters || !summary) return;
    const author = await this.#getClientProfile();
    const current = this.impl.approvalWaiters(chatId);
    if (this.impl.storage.chatMeta.get(chatId)?.activeAgent || !current ||
        current.boundary !== waiters.boundary || current.actions.length !== waiters.actions.length ||
        current.actions.some((action, index) => action.id !== waiters.actions[index]!.id) ||
        approvedActionSummary(current.actions) !== summary) return;
    this.impl.addChatMessages(chatId, author, [{type: "message", message: summary}]);
    await this.#resumeSuspendedAgent(chatId, this.impl.approvalWaiters(chatId)?.boundary);
  }
  ...
  async #resumeSuspendedAgent(chatId: number, approvalBoundary?: number): Promise<void> {
    await this.impl.waitForChatMessagePreparation(chatId);
    if (approvalBoundary !== undefined && this.impl.approvalWaiters(chatId)?.boundary !== approvalBoundary) return;
    ...
    if (preparation) {
      await preparation;
      return this.#resumeSuspendedAgent(chatId, approvalBoundary);
    }
