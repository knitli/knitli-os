...
function makeManager(
    ownerInvitesOnly: () => boolean = () => false, beforeAuthorityExpansion?: () => void)
    : { storage: SharingStorage; mgr: SharingManager } {
  let storage = makeStorage();
  return {
    storage,
    mgr: new SharingManager(storage, OWNER, ownerInvitesOnly, beforeAuthorityExpansion),
  };
}
...
describe("redeemShareKey", () => {
  it("rechecks sharing policy after fetching a new collaborator's profile", async () => {
    ...
    let { storage, mgr } = makeManager(undefined, () => {
...
describe("addCollaborator", () => {
  it("checks sharing policy only when an edge is added or upgraded", () => {
    ...
    let { mgr } = makeManager(undefined, () => { expansionChecks++; });
